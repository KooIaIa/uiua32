...
type ValueDyFn = Box<dyn Fn(Value, Value, usize, &mut Uiua) -> UiuaResult<Value>>;
...
fn prim_dy_fast_fn(
    ...
    Some(match prim {
        ...
        Rotate if !b_filled => Box::new(move |a, mut b, depth, env| {
            env.with_span(span, |env| {
                a.rotate_depth(&mut b, depth, true, env)?;
                Ok(b)
            })
        }),
        _ => return None,
...
pub(crate) fn f_dy_fast_fn(
    ...
    match nodes {
        ...
        &[Node::ImplPrim(prim, span)] => {
            Some((impl_prim_dy_fast_fn(prim, span, a_filled, b_filled)?, 0))
        }
        [Node::Mod(Rows, args, _)] => {
            f_dy_fast_fn(args[0].node.as_slice(), a_filled, b_filled).map(|(f, d)| (f, d + 1))
        }
        [Node::Prim(Flip, _), rest @ ..] => {
            let (f, depth) = f_dy_fast_fn(rest, a_filled, b_filled)?;
            let f = Box::new(move |a, b, depth, env: &mut Uiua| f(b, a, depth, env));
            Some((f, depth))
        }
        _ => None,
    }
...
fn each2(f: SigNode, mut xs: Value, mut ys: Value, env: &mut Uiua) -> UiuaResult {
    if let Some((f, ..)) = f_dy_fast_fn(
        ...
        let xrank = xs.rank();
        let yrank = ys.rank();
        let val = f(xs, ys, xrank.min(yrank), env)?;
        env.push(val);
    } else {
        let outputs = f.sig.outputs();
...
pub fn rows(f: SigNode, depth: usize, inv: bool, env: &mut Uiua) -> UiuaResult {
    crate::profile_function!();
    match f.sig.args() {
        0 => env.without_fill(|env| env.exec(f)),
        1 => rows1(f, env.pop(1)?, depth, inv, env),
        2 => rows2(f, env.pop(1)?, env.pop(2)?, depth, inv, env),
        n => {
            let mut args = Vec::with_capacity(n);
            for i in 0..n {
                args.push(env.pop(i + 1)?);
            }
            rowsn(f, args, depth, inv, env)
        }
    }
}
...
pub fn rows1(f: SigNode, mut xs: Value, depth: usize, inv: bool, env: &mut Uiua) -> UiuaResult {
    if !inv {
...
fn rowsn(f: SigNode, args: Vec<Value>, depth: usize, inv: bool, env: &mut Uiua) -> UiuaResult {
    let outputs = f.sig.outputs();
    let prim = if inv {
        Primitive::Inventory
    } else {
